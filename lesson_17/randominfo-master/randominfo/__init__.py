...
import os
sys.path.append(os.path.abspath('/Users/marianna/PycharmProjects/demo/Lesson_2-GitHub-/lesson_17/randominfo-master'))
...
csv_path = full_path('data.csv')
if not os.path.exists(csv_path):
	print(f"File 'data.csv' missing in folder {csv_path}.")
else:
	print(f"File 'data.csv' found by way of {csv_path}")
...
def get_alphabetic_profile_img(char, filePath, imgName, charColor = None, bgColor = None):
	...
	if all((c in chars) for c in imgName):
		if os.access(dirname(filePath), os.W_OK):
			if charColor != None:
